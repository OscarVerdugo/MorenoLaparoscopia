...
class EmployeesController {
    ...
    if (object.length > 0) {
      if (req.body["cPassword"] == object[0].cPassword) {
        if (!object[0].bOnline) {
          ...
          let resData = await pool.query('SELECT u.cLogin,e.nIdEmpleado,e.cNombre,p.cDescripcion as cDescripcionPuesto FROM usuarios u LEFT JOIN empleados e ON u.nIdEmpleado = e.nIdEmpleado LEFT JOIN puestos p ON p.nIdPuesto = e.nIdPuesto WHERE e.nIdEmpleado = ? ',object[0].nIdEmpleado);
          r.data = resData[0];
  ...
  public async cons(req: Request, res: Response) {
    const r = new Model();
    const object = await pool.query(
      "SELECT e.nIdEmpleado,e.cNombre,pu.cDescripcion as cDescripcionPuesto,pu.nIdPuesto,e.cDireccion,e.cTelefono FROM  empleados e  LEFT JOIN puestos pu ON pu.nIdPuesto = e.nIdPuesto WHERE e.nIdEmpleado = ? AND p.bActivo = true",
      [req.params["id"]]
    );
    if (object.length > 0) {
      r.data = object[0];
  ...
  public async select(req: Request, res: Response) {
    const r = new Model();
    const object = await pool.query(
      "SELECT e.nIdEmpleado,e.cNombre,pu.cDescripcion as cDescripcionPuesto,pu.nIdPuesto,e.cDireccion,e.cTelefono FROM  empleados e LEFT JOIN puestos pu ON pu.nIdPuesto = e.nIdPuesto WHERE e.bActivo = true"
    );
    if (object.length > 0) {
      r.data = object;
      r.message = "Exito";
