...
class CustomersController {
  ...
  public async cons(req: Request, res: Response) {
    ...
    const object = await pool.query(
      "SELECT nIdCliente,cNombre,cDireccion,cNumTel FROM clientes WHERE bActivo = true AND nIdCliente = ?",
      [req.params["id"]]
    );
    if (object.length > 0) {
      r.data = object[0];
  ...
  public async select(req: Request, res: Response) {
    const r = new Model();
    const object = await pool.query(
      "SELECT nIdCliente,cNombre,cDireccion,cNumTel FROM clientes WHERE bActivo = true"
    );
    if (object.length > 0) {
      r.data = object;
      r.message = "Exito";
