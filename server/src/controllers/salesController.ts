...
class SalesController {
  ...
  public async cons(req: Request, res: Response) {
    ...
    const object = await pool.query(
      "SELECT v.dFecha,v.nIdEmpleado,v.bPagado,e.cNombre as cNombreEmpleado,v.nIdCliente,c.cNombre as cNombreCliente,v.nSubTotal,v.nIva,v.nTotal FROM ventas v LEFT JOIN empleados e ON v.nIdEmpleado = e.nIdEmpleado LEFT JOIN clientes c ON v.nIdCliente = c.nIdCliente WHERE v.nIdVenta = ? AND v.bActivo = true",
      [req.params["id"]]
    );
    if (object.length > 0) {
      data.venta = object[0];
  ...
  public async select(req: Request, res: Response) {
    const r = new Model();
    const object = await pool.query(
      "SELECT v.nIdVenta,v.dFecha,v.nIdEmpleado,v.bPagado,e.cNombre as cNombreEmpleado,v.nIdCliente,c.cNombre as cNombreCliente,v.nSubTotal,v.nIva,v.nTotal FROM ventas v LEFT JOIN empleados e ON v.nIdEmpleado = e.nIdEmpleado LEFT JOIN clientes c ON v.nIdCliente = c.nIdCliente WHERE v.bActivo = true"
    );
    if (object.length > 0) {
      r.data = object;
      r.message = "Exito";
