...
class ProductsController {
  ...
  public async cons(req: Request, res: Response) {
    ...
    const object = await pool.query(
      "SELECT p.nIdProducto,p.cDescripcion,p.nPrecio FROM productos p  WHERE p.nIdProducto = ? AND p.bActivo = true",
      [req.params["id"]]
    );
    if (object.length > 0) {
      r.data = object[0];
  ...
  public async select(req: Request, res: Response) {
    const r = new Model();
    const object = await pool.query(
      "SELECT p.nIdProducto,p.cDescripcion,p.nPrecio FROM productos p WHERE p.bActivo = true"
    );
    if (object.length > 0) {
      r.data = object;
      r.message = "Exito";
